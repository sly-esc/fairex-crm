...
export default function InventoryCsvImporter({ uploadUrl, extraFormData, onSuccess, onCancel }: InventoryCsvImporterProps) {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-zinc-400 mb-3">Sube un CSV para actualizar precios, stock y catálogo. Máx 5 MB / 2,000 filas.</p>
      ...
      {csvSuccess ? (
        <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-6 text-center space-y-3">
          ...
          <div className="flex justify-center gap-4 mt-4">
            ...
            {onCancel && (
              <Button onClick={onCancel} variant="outline" className="border-white/10 text-white">
                Cerrar
              </Button>
            )}
          </div>
        ...
        <>
          ...
          <div className="flex justify-end gap-3 pt-2">
            {onCancel && (
              <Button type="button" variant="ghost" onClick={onCancel} disabled={csvLoading} className="text-zinc-400 hover:text-white">
                Cancelar
              </Button>
            )}
            <Button
              ...
              className="bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-600/40 disabled:cursor-not-allowed text-white font-medium transition-colors flex items-center justify-center gap-2"
            >
